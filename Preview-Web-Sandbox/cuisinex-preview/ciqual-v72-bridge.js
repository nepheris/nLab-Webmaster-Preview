...
async function bridge(){
  ...
  try{
    ...
    state.data.meta=state.data.meta||{};state.data.meta.ciqual_bridge={status:'connected',source:'P002 V72 — Base Ingrédients avec CIQUAL',matched_ingredients:matched,ingredient_records:sourceRows.length,mapping_records:mapRows.length,loaded_at:'2026-08-26'};saveData();
    document.dispatchEvent(new CustomEvent('cuisinex:ciqual-ready',{detail:state.data.meta.ciqual_bridge}));
  }catch(e){console.warn('CuisineX Ciqual V72 bridge unavailable',e);state.data.meta=state.data.meta||{};state.data.meta.ciqual_bridge={status:'unavailable',message:String(e)}}
}
bridge();
