import {state,normalize,saveData} from './core.js';
...
function ciqualId(row){
  return row?.relations?.ciqual_id||row?.relations?.ciqual_code||row?.attributs_metier?.ciqual_id||row?.attributs_metier?.code_ciqual||null;
}
...
function nutrition(row){
  const a=row?.attributs_metier||{}, c=row?.contenu||{}, n=a.nutrition||c.nutrition||row?.nutrition||{};
  const p=n.per_100g||n.pour_100g||n.valeurs_100g||null;
  return p?{status:'documented',per_100g:p,source:'Base Ingrédients avec CIQUAL / référentiel V72'}:null;
}
...
async function bridge(){
  for(let i=0;i<100&&!state.data;i++)await new Promise(r=>setTimeout(r,50));
  if(!state.data)return;
  try{
    const [ingredients,mappings]=await Promise.all([fetchJson(INGREDIENTS),fetchJson(MAPPINGS)]);
    const sourceRows=rows(ingredients), mapRows=rows(mappings);
    const byProject=new Map(), byName=new Map();
    ...
    for(const ing of state.data.ingredients||[]){
      ...
      ing.source_type='ciqual';
      ing.source_status='project_ciqual_v72';
      ing.mapping_status=cid?'project_mapping':'project_record';
      if(cid)ing.ciqual_code=cid;
      if(nut)ing.nutrition=n nut;
      ing.ciqual_reference={
        source:'Base Ingrédients avec CIQUAL V2.1 / référentiel V72',
        source_repository:'recettesducoeur/recettesducoeur.github.io',
        ingredient_record_url:`${BASE}ingredients.json`,
        mapping_url:MAPPINGS,
        official_source_file:SOURCE_XLSX,
        project_ingredient_id:pid||null,
        ciqual_id:cid||null,
        confidence:map?.attributs_metier?.niveau_confiance||null,
        validation_status:map?.validation?.statut_validation_id||null
      };
      matched++;
    }
    state.data.meta=state.data.meta||{};
    state.data.meta.ciqual_bridge={status:'connected',source:'P002 V72 — Base Ingrédients avec CIQUAL',matched_ingredients:matched,ingredient_records:sourceRows.length,mapping_records:mapRows.length,loaded_at:'2026-08-26'};
    saveData();
    document.dispatchEvent(new CustomEvent('cuisinex:ciqual-ready',{detail:state.data.meta.ciqual_bridge}));
  }catch(e){
    console.warn('CuisineX Ciqual V72 bridge unavailable',e);
    state.data.meta=state.data.meta||{};
    state.data.meta.ciqual_bridge={status:'unavailable',message:String(e)};
  }
}
bridge();
