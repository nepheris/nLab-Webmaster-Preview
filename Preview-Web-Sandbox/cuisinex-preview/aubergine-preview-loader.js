(() => {
  ...
  window.fetch = async (input, init) => {
    ...
    const [baseCatalog, aubergineCatalog] = await Promise.all([baseResponse.clone().json(), aubergineResponse.json()]);
    const recipes = Array.isArray(baseCatalog.recipes) ? [...baseCatalog.recipes] : [];
    const ids = new Set(recipes.map(r => r.id));
    for (const recipe of aubergineCatalog.recipes || []) if (!ids.has(recipe.id)) { recipes.push(recipe); ids.add(recipe.id); }
    return new Response(JSON.stringify({
      ...baseCatalog,
      meta: { ...(baseCatalog.meta || {}), preview_extensions: [...((baseCatalog.meta || {}).preview_extensions || []), 'RF-AUBERGINE-PULPE:candidate_unvalidated'] },
      recipes
